...
server.get('List', userLoggedIn.validateLoggedIn, consentTracking.consent, function (req, res, next) {
    var actionUrls = {
        ...
        makeDefaultUrl: URLUtils.url('Address-SetDefault').toString()
    };

    const siteUtils = require('app_brooks_custom/cartridge/scripts/util/siteUtils.js');
    let template = 'account/addressBook';
    if (siteUtils.isFeatureEnabled('BRC-117')) {
        template = 'account/pages/addressBook';
    }
...
server.get(
    ...
    function (req, res, next) {
        ...
        // Get the customer's address book to pass to the template
        var customerNo = req.currentCustomer.profile.customerNo;
        var addressBook = getList(customerNo);
...
server.get(
    ...
    function (req, res, next) {
        ...
        // Get the customer's address book to pass to the template
        var customerNo = req.currentCustomer.profile.customerNo;
        var addressBookList = getList(customerNo);
            ...
            Transaction.wrap(function () {
                ...
                if (address) {
                    ...
                    res.json({
                        ...
                        addressDefaultURL: URLUtils.url('Address-SetDefault').toString()
                    });
                } else {
...
server.get('DeleteAddress', userLoggedIn.validateLoggedInAjax, function (req, res, next) {
    ...
    this.on('route:BeforeComplete', function () { // eslint-disable-line no-shadow
        ...
        if (length === 0) {
            ...
            res.json({ UUID: UUID,
                defaultMsg: Resource.msg('label.addressbook.defaultaddress', 'account', null),
                successMsg: Resource.msgf('shipping.address.delete.success', 'checkoutRefresh', null, address.ID)
            });
        }
    });
    return next();
