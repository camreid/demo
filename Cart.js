...
server.post('AddProduct', function (req, res, next) {
    ...
    if (currentBasket) {
        Transaction.wrap(function () {
            if (!req.form.pidsObj) {
                ...
                // product set
                productSetUUID = require('dw/util/UUIDUtils').createUUID();
                ...
                pidsObj.forEach(function (PIDObj) {
                    ...
                    pidOptions.productSetUUID = productSetUUID;
                    pidOptions.productSetID = productId;
                    PIDObjResult = cartHelper.addProductToCart(
                        currentBasket,
                        PIDObj.pid,
                        quantity,
                        childProducts,
    ...
    var uuid = (result && result.uuid) || (PIDObjResult && PIDObjResult.uuid);
...
server.get('RemoveProductLineItem', function (req, res, next) {
    ...
    if (req.querystring.pid && req.querystring.uuid) {
        ...
        var basketPromotions = cartHelper.getPromotionsList(currentBasket.getPriceAdjustments());
        var shippingPromotions = cartHelper.getPromotionsList(currentBasket.getAllShippingPriceAdjustments());
        productPromotions = productPromotions.concat(basketPromotions);
        productPromotions = productPromotions.concat(shippingPromotions);

        for (var i = 0; i < productLineItems.length; i++) {
            var item = productLineItems[i];

            productPromotions = productPromotions.concat(cartHelper.getPromotionsList(item.getPriceAdjustments())); // eslint-disable-line
    ...
    if (productPromotions.length > 0 && !req.querystring.remove) {
        var updatedBasket = BasketMgr.getCurrentBasket();
        var productLineItems = updatedBasket.getAllProductLineItems(); // eslint-disable-line
        var newProductPromotions = [];
        var basketPromotions = cartHelper.getPromotionsList(updatedBasket.getPriceAdjustments()); // eslint-disable-line
        var shippingPromotions = cartHelper.getPromotionsList(updatedBasket.getAllShippingPriceAdjustments()); // eslint-disable-line
        newProductPromotions = newProductPromotions.concat(basketPromotions);
        newProductPromotions = newProductPromotions.concat(shippingPromotions);

        for (var i = 0; i < productLineItems.length; i++) { // eslint-disable-line
            var item = productLineItems[i]; // eslint-disable-line
            var priceAdjustments = item.getPriceAdjustments();
            newProductPromotions = newProductPromotions.concat(cartHelper.getPromotionsList(priceAdjustments));
        }

        removedPromotions = productPromotions.filter(function (promoItem) {
            return newProductPromotions.indexOf(promoItem) === -1;
        });
    ...
    if (isProductLineItemFound) {
        ...
        var basketModelPlus = {
            basket: basketModel,
            removedPromotions: removedPromotions
        };
    ...
    var isProductLineItemFound = false; // eslint-disable-line
    var bonusProductsUUIDs = []; // eslint-disable-line
    ...
    if (isProductLineItemFound) {
        var basketModel = new CartModel(currentBasket); // eslint-disable-line
        var basketModelPlus = { // eslint-disable-line
            basket: basketModel,
...
server.get('RemoveCouponLineItem', function (req, res, next) {
    ...
    if (currentBasket && req.querystring.uuid) {
        ...
        if (couponLineItem) {
            ...
            // clear payment data when applying coupon code on checkout
            var checkoutUpdateHelpers = require('*/cartridge/scripts/checkout/checkoutUpdateHelpers.js');
            checkoutUpdateHelpers.clearPaymentInstruments(currentBasket);
...
server.post('AddBonusProducts', function (req, res, next) {
    ...
    if (totalQty === 0) {
        ...
        if (currentBasket) {
            ...
            // check if related pli exists
            var matchingPLI = collections.find(currentBasket.getAllProductLineItems(), function (item) {
                return !empty(pliUUID) && (item.UUID === pliUUID);
            });

            if (!matchingPLI) {
                // assign bonus to existing pli
                var newPLI = collections.first(currentBasket.getAllProductLineItems(), function (item) {
                    return item.custom.bonusProductLineItemUUID !== 'bonus' && !item.bonusProductLineItem;
                });
                if (newPLI) {
                    pliUUID = newPLI.UUID;
                }
            ...
            Transaction.wrap(function () {
                ...
                for (var i in allProductLineItems) {
                    var productLineItem = allProductLineItems[i];
                    if (productLineItem.UUID === pliUUID) {
                        ...
                        productLineItem.custom.preOrderUUID = pliUUID;// eslint-disable-line no-param-reassign
...
server.get('EditBonusProduct', function (req, res, next) {
    ...
    if (bonusDiscountLineItem) {
        ...
        var pids = req.querystring.pids || collections.pluck(bonusDiscountLineItem.bonusProducts, 'ID').join();
        ...
        res.json({
            ...
            showProductsUrlRuleBased: URLUtils.url('Product-ShowBonusProducts', 'DUUID', bonusDiscountLineItem.UUID, 'pagesize', cartHelper.BONUS_PRODUCTS_PAGE_SIZE, 'pagestart', 0, 'maxpids', bonusDiscountLineItem.maxBonusItems, 'isCartPage', true).toString(),
            showProductsUrlListBased: URLUtils.url('Product-ShowBonusProducts', 'DUUID', bonusDiscountLineItem.UUID, 'pids', pids, 'maxpids', bonusDiscountLineItem.maxBonusItems, 'isCartPage', true).toString()
        });
    } else {
        res.json({});
    }
...
server.get('GetProduct', function (req, res, next) {
    ...
    var context = {
        ...
        redirectUrl:  URLUtils.url('Product-Show', 'pid', pliProduct.pid, 'uuid', requestUuid, 'quantity', requestQuantity).toString(),
...
server.post('EditProductLineItem', function (req, res, next) {
    ...
    var cartHelper = require('*/cartridge/scripts/cart/cartHelpers');
    var basketCalculationHelpers = require('*/cartridge/scripts/helpers/basketCalculationHelpers');
    const { setBasketItemsAvailabilityOnCart } = require('int_oms/cartridge/scripts/util/omsInventoryHelper');
    ...
    if (canBeUpdated) {
        ...
        try {
            Transaction.wrap(function () {
                ...
                // Update all productLineItems' custom attribute omsQtyAvailable
                setBasketItemsAvailabilityOnCart(currentBasket);
                basketCalculationHelpers.calculateTotals(currentBasket);
            });
        } catch (e) {
            error = true;
    ...
    if (!error && requestLineItem && canBeUpdated) {
        ...
        responseObject.redirectUrl = URLUtils.url('Cart-Show').toString();
