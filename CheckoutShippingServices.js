...
server.post('SelectShippingMethod', server.middleware.https, function (req, res, next) {
    ...
    this.on('route:BeforeComplete', function (req, res) { // eslint-disable-line no-shadow
        ...
        var basketModel = new OrderModel(
            currentBasket,
            { usingMultiShipping: usingMultiShipping, countryCode: currentLocale.country, containerView: 'basket', useShippingAddress: true }
        );
...
server.post(
    ...
    function (req, res, next) {
        ...
        if (req.currentCustomer
            ...
            && !empty(req.currentCustomer.profile.email)) {

            if (empty(currentBasket.getCustomerEmail())) {
                Transaction.wrap(function () {
        ...
        if (Object.keys(shippingFormErrors).length > 0) {
            ...
            if (Object.prototype.hasOwnProperty
                .call(form.shippingAddress.addressFields, 'companyName')) {
            ...
            result.shippingMethod = currentBasket.defaultShipment.shippingMethodID;
            ...
            this.on('route:BeforeComplete', function (req, res) { // eslint-disable-line no-shadow
                ...
                // copy billing address from shipping if there is no billing address or if its country code deosn't match current country
                var checkoutUpdateHelpers = require('*/cartridge/scripts/checkout/checkoutUpdateHelpers.js');
                var doCpoyBillingAddress = checkoutUpdateHelpers.doCopyBillingAddress(currentBasket.billingAddress);
                if (doCpoyBillingAddress) {
                ...
                shippingData.shippingMethodsError = false;

                if (basketModel.shipping && basketModel.shipping[0] && basketModel.shipping[0].applicableShippingMethods.length === 0) {
                    shippingData.shippingMethod = basketModel.shipping[0].selectedShippingMethod;
                    shippingData.shippingMethodsError = true;
                    shippingData.shippingMethodsMsg = Resource.msg('error.applepay', 'cart', null);
                }
