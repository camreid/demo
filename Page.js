...
server.get('SetLocale', function (req, res, next) {
    ...
    if (req.setLocale(req.querystring.code)) {
        ...
        // Fix for SEO URLs when Pipeline alias is set for Search-Show controller
        if (req.querystring.action === 'Search-Show' && Object.hasOwnProperty.call(queryStringObj, 'cgid')) {
            redirectUrl = URLUtils.url(req.querystring.action, 'cgid', queryStringObj.cgid).toString();
...
server.get('Show', cache.applyDefaultCache, consentTracking.consent, function (req, res, next) {
    ...
    if (page != null && page.isVisible()) {
        ...
        if (apiContent) {
            ...
            var analyticsData = require('*/cartridge/scripts/helpers/analyticsDataHelper.js').getContentAnalyticsData(content);

            if (content.template) {
                res.render(content.template, { content: content, analyticsData: analyticsData });
            } else {
                Logger.warn('Content asset with ID {0} is offline', req.querystring.cid);
                res.render('/components/content/offlineContent');
            }
...
server.get('IncludeBaseTrackingData', function (req, res, next) {
    var features = require('*/cartridge/scripts/helpers/features.js').configuration;

    var analyticsDataHelper = require('*/cartridge/scripts/helpers/analyticsDataHelper.js');
    var baseData = analyticsDataHelper.baseData;
    var userId = empty(baseData.customerNo) ? '' : baseData.customerNo;

    var siteUtils = require('*/cartridge/scripts/util/siteUtils.js');
    var isBRCEmea = siteUtils.isBRCEmeaRegion();

    res.render('common/baseTrackingData.isml', {
        baseData: baseData,
        userId: userId,
        isTealiumEnabled: features.isTealiumEnabled(),
        isBRCEmea: isBRCEmea
    });
