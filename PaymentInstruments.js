...
function verifyCard(req, card, form) {
    ...
    if (paymentCard) {
        ...
        formCardNumber.valid = false;
        formCardNumber.error = Resource.msg('error.not.supported.card', 'creditCard', null);
        error = true;
    }
...
server.get(
    ...
    function (req, res, next) {
        ...
        res.render('account/payment/paymentForm', {
            paymentForm: paymentForm,
 ...
 * @name Base/PaymentInstruments-SetDefault
 * @function
 * @memberof PaymentInstrument
 * @param {middleware} - userLoggedIn.validateLoggedIn
 * @param {querystringparameter} - paymentUUID - a string used to identify the payment instrument record
 * @param {category} - sensitive
 * @param {serverfunction} - get
 */
server.get('SetDefault', userLoggedIn.validateLoggedIn, function (req, res, next) {
    ...
    accountHelpers.setDefaultPaymentInstrument(req.currentCustomer, req.querystring.paymentUUID);
