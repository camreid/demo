...
var sizeGuide = require('*/cartridge/scripts/product/sizeGuide');
var pageMetaHelper = require('*/cartridge/scripts/helpers/pageMetaHelper');

server.get('SizeGuide', cache.applyDefaultCache, function (req, res, next) {
    var ContentUtils = require('*/cartridge/scripts/helpers/contentUtils');
    var ContentModel = require('*/cartridge/models/content');
    var sizeGuideFoldersStructure = sizeGuide.getSizeGuideFoldersStructure(req.querystring.sizeChartIdNew || '');
    var selectedContent = sizeGuide.getSelectedContent(sizeGuideFoldersStructure);
    var sizeGuideTemplate = 'product/components/sizeChartTemplateWithDecorator';
    var isAjaxCall = req.querystring.ajaxCall === 'true';

    if (req.querystring.getSelectedContent == 'true') {
        sizeGuideTemplate = 'product/components/sizeChartContent';
    }

    if (isAjaxCall) {
        sizeGuideTemplate = 'product/components/sizeChartTemplate';
    }

    var apiContent = ContentUtils.getMetaDataContent('Product-SizeGuide');
    if (apiContent) {
    ...
    var hreflangTags = require('app_brooks_custom/cartridge/scripts/util/localeUtil').getLinksForAllLocales(
        'Product-SizeGuide',
        req.querystring
    );

    var canonicalUrl = require('*/cartridge/scripts/util/canonicalLink').getCanonicalLink('Product-SizeGuide');

    res.render(sizeGuideTemplate, {
        sizeGuideFoldersStructure: sizeGuideFoldersStructure,
...
server.get('Show', cache.applyPromotionSensitiveCache, consentTracking.consent, function (req, res, next) {
    ...
    var cartItemUUID = req.querystring.uuid;
    var requestQuantity = req.querystring.quantity;
...
server.get('Variation', function (req, res, next) {
    ...
    const memberExclusiveHelpers = require('*/cartridge/scripts/helpers/memberExclusiveHelpers');
    ...
    if (require('*/cartridge/scripts/util/siteUtils').isFeatureEnabled('DPEN-1308')) {
        try {
            product.price.html = priceHelper.renderHtml(priceHelper.getHtmlContext(context));
        } catch (error) {
            const Logger = require('dw/system/Logger');
            Logger.error('Error "{0}" rendering Product-Variation for "{1}", referred by {2}: {3}, {4}', error.name, params.pid, request.httpReferer || 'none', error.message, error.stack);
            res.json({
                error: true
            });
            return next();
    ...
    var promotionsContext = { product: product, isMember: memberExclusiveHelpers.isMember(product.memberExclusiveCustomerGroup) };
...
server.get('ShowBonusProducts', function (req, res, next) {
    ...
    if (duuid) {
        ...
        if (bonusDiscountLineItem && bonusDiscountLineItem.bonusProductLineItems.length) {
            selectedBonusProducts = collections.map(bonusDiscountLineItem.bonusProductLineItems, function (bonusProductLineItem) {
                ...
                return {
                    ...
                    masterID: bonusProductLineItem.product && bonusProductLineItem.product.variant && bonusProductLineItem.product.masterProduct ? bonusProductLineItem.product.masterProduct.ID : bonusProductLineItem.productID,
        ...
        if (req.querystring.pids) {
            ...
            params.forEach(function (param) {
                if (bonusDiscountLineItem.bonusProductLineItems.length > 0) {
                    var selectedBonusProductVariationModel = null;
                    var selectedBonusProductVariation = bonusDiscountLineItem.bonusProductLineItems[0].product;
                    if (selectedBonusProductVariation.variant && selectedBonusProductVariation.masterProduct.ID === param) {
                        selectedBonusProductVariationModel = selectedBonusProductVariation.variationModel;
                    }
                }
                product = ProductFactory.get({
                    ...
                    pview: 'bonus',
                    duuid: duuid,
                    selectedBonusProductVariationModel: selectedBonusProductVariationModel });
    ...
    var context = {
        ...
        template: 'product/components/addToCartBonusProduct.isml',
