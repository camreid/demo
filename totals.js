...
const { customFormatMoney } = require('*/cartridge/scripts/util/formatting');
...
function getTotals(total) {
    return !total.available ? '-' : customFormatMoney(total);
}
...
function getOrderLevelDiscountTotal(lineItemContainer) {
    ...
    return {
        ...
        formatted: customFormatMoney(orderDiscount)
    };
}
...
function getShippingLevelDiscountTotal(lineItemContainer) {
    ...
    return {
        ...
        formatted: customFormatMoney(shippingDiscount)
    };
}
    ...
    collections.forEach(collection, function (item) {
        if (!item.basedOnCoupon) {
            result[item.UUID] = {
                ...
                price: customFormatMoney(item.price),
                type: 'promotion',
